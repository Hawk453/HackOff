...
We implemented a portal for the people to facilitate them in storage of any kind of Confidential Data. We have used HTML, CSS, Javascript, Django, AES Algorithm, ARGON2, Optical Character Recognition(OCR) for building the portal, encryption of data and their features. Along with them we used Python for building up the Optical Character Recognition feature which has been a way to encrypt the data in particular kind of files like scanned PDFs and Pictures. We did the encryption of the data in the files using the AES Algorithm via Hashing to generate a key using ARGON2 which was the winner of The Password Hashing Competition(PHC) from a salt and initialisation vector. To retrieve the same file we use Salt and IV (Initialisation Vector) with the user input password to generate a key. The key is in no manner is stored anywhere across the platform. The Files that we can encrypt are text files and scanned documents.
